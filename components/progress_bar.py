...
class ProgressBar(pg.sprite.DirtySprite):
    ...
    def redraw(self):
        ...
        self.unfill_rect.width -= (1-(self.__value / self.__max_value)) * self.unfill_rect.width

        pg.draw.rect(self.image, self.background_color, self.unfill_rect)

        text_s = self.font.render(self.text_format.replace('{0}', str(int((self.__value / self.__max_value)*100))+'%'), True, (0,0,0))

        self.image.blit(text_s, (self.image.get_width()/2 - text_s.get_width()/2, (self.image.get_height()/2 - text_s.get_height()/2)))

        text_s = self.font.render(self.text_format.replace('{0}', str(int((self.__value / self.__max_value)*100))+'%'), True, (255,255,255))
        text_sur = pg.Surface(self.image.get_size(), pg.SRCALPHA)

        text_sur.blit(text_s, (self.image.get_width()/2 - text_s.get_width()/2, (self.image.get_height()/2 - text_s.get_height()/2) + self.border_width ))
