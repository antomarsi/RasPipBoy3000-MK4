...
class IntroScene(SceneBase):
    ...
    def __init__(self, skip_intro=False):
        ...
        self.fontX2 = ResourceLoader.getInstance().get_font("MONOFONTO", 16)
        ...
        pg.time.delay(1500)
        ResourceLoader.getInstance().play_sound('sounds/boot/a.ogg')
        pg.time.delay(800)
    ...
    def roll_text(self, dt):
        if self.text_y < ((self.surface.get_height())*-1):
            self.state = 1
            ResourceLoader.getInstance().play_sound('sounds/boot/b.ogg')
            pg.time.delay(500)
        else:
    ...
    def set_download_text(self, value):
        self.text_download = ResourceLoader.getInstance().get_font("MONOFONTO", 12).render(
    ...
    def load_files(self):
        ...
        ResourceLoader.getInstance().play_sound('sounds/boot/c.ogg')
        ...
        for img in files_images:
            ...
            ResourceLoader.getInstance().get_image(img)
            value += 1
            self.download_progress.set_value(value)
        print("All images loaded")
        for img in files_sounds:
            img = str(img).replace(cfg.assets_folder+"/", '')
            ResourceLoader.getInstance().get_sound(img)
            value += 1
            self.download_progress.set_value(value)
        ResourceLoader.getInstance().play_sound('sounds/UI_PipBoy_Map_Rollover_01.ogg')
    ...
    def update(self, dt):
        ...
        if self.state == 3:
            ...
            if (not self.thread_load_files.isAlive() and self.vault_boy.finished):
                ResourceLoader.getInstance().play_music('sounds/UI_PipBoy_Hum_LP.wav')
                self.state = 4
        pass
